...
void Write_IIC_Byte(uint8_t IIC_Byte)
{
		...
		for(i=0;i<8;i++)		
		{
			...
			if(m==0x80)
					...
					OLED_SDIN_Clr();
			da=da<<1;
			//delay_us(1);//和下面的__NOP();选一个留下，如果单片机主频低于72MHZ可以不延时
...
//num:数值(0~4294967295);	 		  
//Color_Turn:颜色翻转
void OLED_ShowNum(uint8_t x,uint8_t y,uint32_t num,uint8_t len,uint8_t size2,uint8_t Color_Turn)
{         	
	uint8_t t,temp;
	uint8_t enshow=0;						   
	for(t=0;t<len;t++)
...
{ 	
		...
		__HAL_RCC_GPIOC_CLK_ENABLE();//注意修改
	 
		HAL_GPIO_WritePin(OLED, OLED_SCL|OLED_SDA, GPIO_PIN_RESET);//设置SCL，SDA
		/*Configure GPIO pins : PB10 PB11 */
		GPIO_InitStruct.Pin = OLED_SCL|OLED_SDA;
		GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
		GPIO_InitStruct.Pull = GPIO_PULLUP;
		GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
		HAL_GPIO_Init(OLED, &GPIO_InitStruct);
...
{
	uint8_t system_clock = 168;//单片机主频
...
//来自 <https://www.pudn.com/news/623af7cc49c1dc3c89808034.html> 
