...
extern UART_HandleTypeDef stm32_huart;
/**
...
{
  if(HAL_UART_Receive (&stm32_huart ,(uint8_t *)getbyte,1,0x01) != HAL_OK )
  {
		return HAL_ERROR;
	}
	else
...
/**
  * @brief  中断处理函数，在串口接收中断中调用：将寄存器的数值压入缓冲区
  * @param
  * @note
  * @retval void
  * @author xiaodaqi
...
{
 if(HAL_UART_Transmit(&stm32_huart ,(uint8_t *)buf,length,0xff))
 {
   return HAL_ERROR;
 }
 	else
